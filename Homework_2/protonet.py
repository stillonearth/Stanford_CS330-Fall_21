...
class ProtoNet:
    ...
    def _compute_prototype(self, x, y):
        labels, _ = torch.sort(torch.unique(y))
        prototypes = torch.zeros((labels.size()[0], x.size()[1])).to(DEVICE)
        prototypes_counts = {}
        ...
        for i, label in enumerate(y):
            ...
            prototypes[l] += x[i]
            if l not in prototypes_counts: prototypes_counts[l] = 0
            prototypes_counts[l] += 1

        for i, count in prototypes_counts.items():
            prototypes[i] /= count
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            y_s = self._network(images_support)
            y_q = self._network(images_query)

            with torch.no_grad():
                prototypes = self._compute_prototype(y_s, labels_support)

            distance_1 = lambda v1, v2: ((v1-v2)**2).sum(axis=0)
            distance_2 = lambda v1, v2: ((v1-v2)**2).sum(axis=1)

            loss = []
            for i, y in enumerate(labels_query):

                a = distance_1(prototypes[y], y_q[i])
                b = distance_2(prototypes, y_q[i])

                prob = torch.exp(-a) /  torch.sum(torch.exp(-b))                
                log_prob = -torch.log(prob)
                loss.append(log_prob)

            pred_s = []
            for i in range(y_s.size()[0]):
                pred_s.append(F.softmax(-distance_2(y_s[i], prototypes), dim=0))
            pred_s = torch.stack(pred_s)
            
            pred_q = []
            for i in range(y_q.size()[0]):
                pred_q.append(F.softmax(-distance_2(prototypes, y_q[i]), dim=0))
            pred_q = torch.stack(pred_q)

            loss_batch.append(torch.nanmean(torch.stack(loss)))
            accuracy_support_batch.append(util.score(pred_s, labels_support))
            accuracy_query_batch.append(util.score(pred_q, labels_query))
        ...
        return (
            torch.nanmean(torch.stack(loss_batch)),
